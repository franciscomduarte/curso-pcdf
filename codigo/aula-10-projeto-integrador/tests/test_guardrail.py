"""Testes do guardrail — herdado sem mudança da Aula 9."""
...
def test_origens_diferentes_tem_cotas_independentes():
    ...
    assert g.permitir("origem-a") is True
    assert g.permitir("origem-b") is True
...
def test_janela_expira_e_libera_cota_de_novo():
    ...
    assert g.permitir("origem-a", agora=5.0) is False
    assert g.permitir("origem-a", agora=11.0) is True
