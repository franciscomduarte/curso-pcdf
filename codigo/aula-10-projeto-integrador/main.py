"""Nível básico: o SIGMA completo — cluster (Aula 8/9) + roteador condicional
(Aula 6) + guardrail/Gateway (Aula 9/1) + HPA (Aula 9) + métricas (Aula 9).

    python main.py

Mostra, nesta ordem: (1) dois boletins pelo Gateway — um Roubo, roteado para
revisão dupla antes do breakpoint; um Furto, direto ao breakpoint, como
sempre foi; (2) um boletim com injeção sendo barrado pelo guardrail; (3) uma
origem estourando o limite de taxa; (4) o HPA do investigador reagindo a uma
carga simulada; (5) o painel de métricas final.
"""
...
def montar_cluster(metricas: Metricas) -> Cluster:
    ...
    c.criar_service("revisor-svc", "revisor-deploy")
...
def processar_e_aprovar(gateway: Gateway, oc_id: str, texto: str, origem: str) -> None:
    ...
    etapas = [p["etapa"] for p in r2.estado.atendido_por]
    print(f"  {oc_id}: {r2.estado.dossie['tipificacao']['artigo']} "
          f"— rota: {' -> '.join(etapas)}")
    if r2.estado.revisao is not None:
        print(f"    revisão dupla: {r2.estado.revisao['observacao']}")
...
def main() -> None:
    ...
    print("=== 1. Dois boletins pelo Gateway — rotas diferentes ===")
