"""Testes do HPA mínimo — a fórmula real do Kubernetes, em isolamento."""
...
def test_carga_dentro_do_alvo_nao_escala():
    # carga_atual é a média POR PODE já calculada (como o metrics-server real
    # devolve) — se ela bate o alvo, a contagem de réplicas não muda.
    c = _cluster_com_deployment(replicas=2)
    hpa = HPA(nome="h", deployment="d", alvo_por_pod=5.0, min_replicas=1, max_replicas=10)
    r = avaliar(c, hpa, carga_atual=5.0)
