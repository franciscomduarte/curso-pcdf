"""Motor mínimo de cluster — o mesmo da Aula 8, agora medindo o que acontece.

A única mudança estrutural em relação à Aula 8: `chamar()` cronometra a
imagem e, se um `Metricas` foi passado ao Cluster, registra latência e
sucesso/falha ali. Nada na API pública muda para quem só usava
`criar_deployment`/`criar_service`/`escalar`/`reconciliar` — a Aula 9 é
