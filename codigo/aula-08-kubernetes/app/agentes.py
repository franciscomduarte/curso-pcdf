"""Os quatro agentes especializados — a mesma lógica da Aula 7.

O que muda: cada função agora recebe também `config` (o `ConfigMap` do
Deployment que a hospeda). É assim que a mesma imagem se comporta diferente
em dev/produção, ou por região, sem recompilar nada — só troca o ConfigMap.
...
def juridico(e: Estado, config: dict | None = None) -> Estado:
    """`config["detalhe"]` (do ConfigMap `juridico-config`) controla o nível de
    fundamentação anotado — "padrao" ou "verboso". Não afeta `requer_decisao_humana`:
    esse campo é sempre True, por design (Aula 7, CLAUDE.md §19) — não é algo
    que um ConfigMap deveria poder desligar."""
    config = config or {}
